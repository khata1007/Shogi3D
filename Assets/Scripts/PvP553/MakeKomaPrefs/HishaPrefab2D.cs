...
namespace PvP553.MakeKomaPrefs
{
    [RequireComponent(typeof(MeshRenderer))]
    [RequireComponent(typeof(MeshFilter))]

    public class HishaPrefab2D : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            for (int i = 0; i < 3; i++) sunpou[i] = PvP553.Koma.sunpou[6, i];

            float[] degs = new float[3];
            for (int i = 0; i < 3; i++) degs[i] = PvP553.Koma.deg[6, i];
